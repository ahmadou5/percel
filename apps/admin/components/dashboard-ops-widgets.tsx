...
export function TopExecutiveMetricCards({
  ...
  // Use real computed values if not 24h, else use KPI if available
...
export function OrdersStatusBreakdownWidget({ orders }: { orders: AdminOrder[] }) {
  ...
  const filteredOrders = orders.filter((o) => {
    if (!o.date) return true;
    const orderDate = new Date(o.date);
    if (isNaN(orderDate.getTime())) return true; // Fallback if format is not parseable

    const diffHours = (now.getTime() - orderDate.getTime()) / (1000 * 60 * 60);
    if (range === '24h') return diffHours <= 24;
    if (range === '7d') return diffHours <= 24 * 7;
    return diffHours <= 24 * 30; // 30d
  });
...
export function RidersFleetPanel({ drivers }: { drivers: AdminDriver[] }) {
  ...
  const topRiders = [...drivers].sort((a, b) => {
    // Sort by rating if available, or just fallback to active status
  ...
  return (
    <div className="rounded-2xl border border-border/80 bg-card p-5 shadow-xs space-y-4">
      ...
      <div className="grid grid-cols-3 gap-2 text-xs">
        ...
        <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-2.5">
          <span className="text-[10px] text-emerald-400 uppercase font-bold">Verified Drivers</span>
        ...
        <div className="rounded-xl border border-rose-500/20 bg-rose-500/5 p-2.5">
          <span className="text-[10px] text-rose-400 uppercase font-bold">Action Required</span>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-xs">
          ...
          <tbody>
            {topPending.length > 0 ? topPending.map((sub: any) => (
...
export function GeographicDistributionTable({ hubs = [], serviceAreas = [], orders = [] }: { hubs?: any[], serviceAreas?: any[], orders?: AdminOrder[] }) {
  ...
  // If no hubs are returned from API, fallback to building it from orders
  orders.forEach((o) => {
  ...
  }).sort((a, b) => b.orders - a.orders); // Sort by highest volume
      ...
      <div className="flex items-center justify-between border-b border-border/70 pb-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="rounded-full bg-emerald-500/10 border border-emerald-500/30 px-2 py-0.5 text-[9px] font-bold text-emerald-400 uppercase">
              Live API Data
            </span>
          </div>
...
export function RecentActivityAlertsFeed({ notifications = [] }: { notifications?: AdminNotification[] }) {
  ...
  // Keep feed in sync if props change
  useEffect(() => {
