...
export class OrderService {
  ...
  async getQuote(size: OrderSize, pickupAddress: string, deliveryAddress: string): Promise<OrderQuote> {
    const pickup = await geocodeAddress(cleanText(pickupAddress) ?? pickupAddress);
    const delivery = await geocodeAddress(cleanText(deliveryAddress) ?? deliveryAddress);
    ...
    const quoteKey = `cache:quote:${size}:${route.distanceKm.toFixed(1)}:${Math.round(route.durationMin)}:${onlineDrivers}`;
    const cached = await getCachedJson<OrderQuote>(this.app.redis, quoteKey);
    if (cached) return cached;

    const quote = getPriceQuote(size, route.distanceKm, route.durationMin, onlineDrivers);
    await setCachedJson(this.app.redis, quoteKey, quote, 300);
    return quote;
  }

  async createOrder(userId: string, data: { size: OrderSize; pickupAddress: string; deliveryAddress: string; items: Array<{ description: string; quantity: number; weightKg: number; fragile?: boolean; imageUrl?: string | null }>; notes?: string | null; fragile?: boolean; }) {
