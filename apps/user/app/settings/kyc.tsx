...
import { BadgeCheck, ChevronLeft, ShieldCheck, ScanFace } from 'lucide-react-native';
...
export default function KycScreen() {
  ...
  const submit = async () => {
    if (!canSubmit) {
      Alert.alert('Complete the form', 'Add your address, date of birth, NIN, BVN, and consent before continuing.');
      return;
    }
    ...
    try {
      ...
      const shouldVerifyNin = !updatedProfile.ninVerified || updatedProfile.ninNumber !== nin.trim();
      const shouldVerifyBvn = !updatedProfile.bvnVerified || updatedProfile.bvnNumber !== bvn.trim();
      ...
      Alert.alert(
        ...
        nextProfile.kycComplete
          ...
          : 'Your details were saved. Complete the remaining verification steps to unlock wallet features.',
      );
      router.back();
    } catch (error) {
  ...
  return (
    <ScrollView style={[styles.screen, { backgroundColor: palette.bg }]} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      <View style={[styles.hero, { backgroundColor: palette.primaryDark }]}>
        ...
        <Text style={styles.heroBody}>We use your address, date of birth, NIN, and BVN to create your dedicated NUBAN and to keep bank transfers compliant.</Text>
      </View>

      <View style={[styles.card, { backgroundColor: palette.card, borderColor: palette.border }]}>
        <StateCard
          title={kycComplete ? 'Profile verified' : 'Verification required'}
          description={kycComplete ? 'Your KYC profile is ready for deposits and bank payouts.' : 'Fill in every field below to unlock dedicated account creation and bank transfers.'}
        ...
        <View style={styles.chipGrid}>
          ...
          <View style={[styles.statusChip, { backgroundColor: profile?.ninVerified ? 'rgba(48,209,88,0.12)' : 'rgba(255,214,10,0.12)', borderColor: profile?.ninVerified ? palette.success : palette.warning }]} >
            <Text style={[styles.statusChipLabel, { color: profile?.ninVerified ? palette.success : palette.warning }]}>NIN</Text>
            <Text style={[styles.statusChipValue, { color: palette.text }]}>{profile?.ninVerified ? 'Verified' : 'Pending'}</Text>
          </View>
          <View style={[styles.statusChip, { backgroundColor: profile?.bvnVerified ? 'rgba(48,209,88,0.12)' : 'rgba(255,214,10,0.12)', borderColor: profile?.bvnVerified ? palette.success : palette.warning }]} >
            <Text style={[styles.statusChipLabel, { color: profile?.bvnVerified ? palette.success : palette.warning }]}>BVN</Text>
            <Text style={[styles.statusChipValue, { color: palette.text }]}>{profile?.bvnVerified ? 'Verified' : 'Pending'}</Text>
          </View>
        </View>
        ...
        <Input
          ...
          helperText="Enter the 11 digit NIN linked to your identity."
        />
        ...
        <Input
          ...
          keyboardType="number-pad"
          helperText="Enter the 11 digit BVN linked to your bank account."
        />
